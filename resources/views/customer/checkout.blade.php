    ...
    <div x-data="{ isOpenAddress: false, isOpenEkspedisi: false }" class="container mt-3 mx-auto px-4">
        ...
        <div class="bg-white p-4 rounded shadow-sm mb-2 border-b border-t border-rose-500">
            <div class="flex justify-between items-center">
                <div class="">
                    <h1 class="text-red-500 font-semibold ml-4 mb-3 text-xl"><i class="fa-solid fa-location-dot mr-2"></i>
                        Alamat Pengiriman</h1>
                    @if (session('alamat_baru'))
                        <p class="font-semibold">{{ session('alamat_baru')['name'] }} (+62)
                            {{ session('alamat_baru')['no_telp'] }}</p>
                        <p>{{ session('alamat_baru')['alamat'] }}</p>
                    @else
                        <p class="font-semibold">{{ session('customer')->name }} (+62) {{ session('customer')->no_telp }}
                        </p>
                ...
                @foreach ($keranjangs as $keranjang)
                    <div class="flex items-center mb-2 border-b border-gray-100 pb-3">
                        <div class="flex-shrink-0 bg-gray-200 rounded-lg items-center text-center p-2">
                            <img src="{{ asset('images/' . $keranjang->barang->foto_barang) }}" alt="Product Image"
                        ...
                        <div class="ml-4 flex-grow">
                            ...
                            <div class="flex items-center mt-2">
                                @if ($keranjang->barang->diskon > 0)
                                    <span class="bg-red-500 text-white text-xs px-2 py-1 rounded mr-2">Diskon
                                        {{ $keranjang->barang->diskon }}%</span>
                                @elseif ($keranjang->barang->potongan > 1000)
                                    <span class="bg-red-500 text-white text-xs px-2 py-1 rounded">Potongan Rp.
                                        {{ number_format($keranjang->barang->potongan, 0, ',', '.') }}</span>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col h-full">
                        ...
                        <div class="mb-6">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button @click="isOpenEkspedisi = true"
                                    class="text-blue-500 font-semibold hover:text-blue-700">Ubah</button>
                            </div>
                ...
                <div class="flex-none w-80 p-4 rounded-lg relative">
                    <div class="flex flex-col h-full max-h-42">
                                ...
                                <ul class="space-y-2 max-h-28">
                                    @foreach ($keranjangs as $nota)
                                        <li class="flex justify-between border-b border-gray-200 pb-2">
                                            ...
                                            <span>Rp
                                                {{ number_format($nota->barang->harga * $nota->kuantitas, 0, ',', '.') }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <div class="flex justify-between">
                            ...
                            @endphp
                            <span class="mr-4">Rp
                                {{ number_format($ekspedisi->harga_ekspedisi ?? '', 0, ',', '.') }}</span>
                        </div>
                        <hr class="my-2">
                        <div class="text-right font-semibold text-xl text-red-500">
                            <span>Rp
                                {{ number_format($totalHarga + $ekspedisi->harga_ekspedisi ?? '', 0, ',', '.') }}</span>
                        </div>
                        <div class="mt-4">
                            <button class="w-full bg-red-500 text-white font-semibold py-3 rounded hover:bg-red-700">Buat
                                Pesanan</button>
                        </div>
                    </div>
                </div>
        ...
        {{-- modals untuk alamat --}}
        <div x-show="isOpenAddress" x-cloak
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
                <div class="flex justify-between min-w-full">
                    ...
                    <form action="{{ route('reset.alamat') }}" method="POST" id="reset-form">
                        @csrf
                        <button type="submit"
                            class="bg-yellow-400 py-2 px-4 rounded-md text-gray-800 hover:bg-yellow-500"><i
                ...
                <form action="{{ route('update.alamat') }}" method="POST" id="address-form">
                    ...
                    <div class="flex justify-between">
                        <div class="mb-4">
                            ...
                            <input type="text" id="name" name="name"
                                class="mt-1 block w-full px-8 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-400 sm:text-sm"
                                required>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="alamat" name="alamat"
                            class="mt-1 block w-full px-3 pb-14 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-400 sm:text-sm"
                            placeholder="Kota , Kec, Jl, No.rumah" required>
                    </div>

                    <div class="flex justify-between text-sm">
                        <button type="button" @click="isOpenAddress = false"
                        ...
                        <button type="submit" form="address-form"
                            class="bg-pink-400 py-2 px-4 rounded-md text-white hover:bg-pink-700">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
